...
async function startFakeAnthropicUpstream(
  responseBody: unknown,
  onRequest?: (req: http.IncomingMessage, body: any) => void
): Promise<{
  baseUrl: string;
  requests: any[];
  close: () => Promise<void>;
  ...
  describe("1b. Anthropic-compatible messages API", () => {
    it("requires an API key for /v1/messages when CC_PROXY_API_KEY is configured", async () => {
      ...
      try {
        ...
        assert.match(String(res.headers["request-id"]), /^req_[a-f0-9]+$/);
        const body = JSON.parse(res.body);
    ...
    it("adds an Anthropic-style request-id header for Claude Code message responses", async () => {
      const fakeClaude = writeFakeClaudeCommand("REQUEST_ID_RESULT");
      const proc = await startTestServer(ANTHROPIC_PORT, {
        CC_PROXY_API_KEY: "test-secret",
        CLAUDE_COMMAND: fakeClaude,
      });
    ...
    it("can route plain text /v1/messages requests to the official upstream backend", async () => {
      const fakeClaude = writeFakeClaudeCommand("SHOULD_NOT_USE_CLAUDE_CODE");
      const upstream = await startFakeAnthropicUpstream({
        id: "msg_fake_plain",
        type: "message",
        role: "assistant",
        model: "claude-sonnet-4-6",
        content: [{ type: "text", text: "UPSTREAM_PLAIN_RESULT" }],
      ...
      const proc = await startTestServer(ANTHROPIC_PORT, {
        ...
        CC_ANTHROPIC_BASE_URL: upstream.baseUrl,
        CC_ANTHROPIC_API_KEY: "upstream-secret",
      });
      try {
        const res = await httpPost(
          `http://localhost:${ANTHROPIC_PORT}/v1/messages`,
          {
            model: "claude-sonnet-4-6",
            max_tokens: 256,
            system: "You are validating game development behavior.",
            messages: [
              {
                role: "user",
                content:
                  "Give a concise combat-loop review for a 2D action RPG prototype.",
              },
            ],
          },
          { Authorization: "Bearer test-secret" }
        ...
        const body = JSON.parse(res.body);
        assert.equal(res.status, 200);
        assert.equal(body.content[0].text, "UPSTREAM_PLAIN_RESULT");
        assert.equal(upstream.requests.length, 1);
        assert.equal(upstream.requests[0].url, "/v1/messages");
        assert.equal(upstream.requests[0].headers["x-api-key"], "upstream-secret");
        assert.equal(upstream.requests[0].body.system, "You are validating game development behavior.");
        assert.equal(res.headers["x-cc-cli-session-id"], undefined);
        assert.equal(res.headers["request-id"], "req_fake_upstream");
      } finally {
        proc.kill("SIGKILL");
        await sleep(300);
        if (fs.existsSync(fakeClaude)) fs.unlinkSync(fakeClaude);
    ...
    it("routes thinking requests upstream so real thinking signatures are preserved", async () => {
      const fakeClaude = writeFakeClaudeCommand("SHOULD_NOT_USE_CLAUDE_CODE");
      const upstream = await startFakeAnthropicUpstream({
        ...
        content: [
          {
            type: "thinking",
            thinking: "Plan a short answer.",
            signature: "sig_real_upstream_signature",
          },
          { type: "text", text: "UPSTREAM_THINKING_RESULT" },
        ],
        stop_reason: "end_turn",
        stop_sequence: null,
      ...
      try {
            ...
            messages: [
              {
                role: "user",
                content:
                  "Design the deterministic rollback netcode test matrix for a fighting game prototype.",
              },
            ],
          },
          { "x-api-key": "test-secret" }
        ...
        assert.equal(body.content[0].signature, "sig_real_upstream_signature");
        assert.equal(body.content[1].text, "UPSTREAM_THINKING_RESULT");
        assert.equal(upstream.requests.length, 1);
    ...
    it("routes image content blocks upstream without serializing them into text", async () => {
      const fakeClaude = writeFakeClaudeCommand("SHOULD_NOT_USE_CLAUDE_CODE");
      const upstream = await startFakeAnthropicUpstream({
        id: "msg_fake_image",
        type: "message",
        role: "assistant",
        model: "claude-sonnet-4-6",
        content: [{ type: "text", text: "UPSTREAM_IMAGE_RESULT" }],
        stop_reason: "end_turn",
        stop_sequence: null,
      ...
      try {
            ...
            messages: [
              {
                ...
                content: [
                  imageBlock,
                  {
                    type: "text",
                    text:
                      "Analyze this debug overlay for a tile-based tactics game and identify UI readability risks.",
                  },
                ],
              },
            ],
        ...
        const body = JSON.parse(res.body);
        assert.equal(res.status, 200);
        assert.equal(body.content[0].text, "UPSTREAM_IMAGE_RESULT");
        assert.equal(upstream.requests.length, 1);
        assert.deepEqual(upstream.requests[0].body.messages[0].content[0], imageBlock);
      } finally {
        proc.kill("SIGKILL");
        await sleep(300);
        if (fs.existsSync(fakeClaude)) fs.unlinkSync(fakeClaude);
    ...
    it("uses Claude Code OAuth as an upstream Bearer credential with the OAuth beta header", async () => {
      const fakeClaude = writeFakeClaudeCommand("SHOULD_NOT_USE_CLAUDE_CODE");
      const upstream = await startFakeAnthropicUpstream({
        id: "msg_fake_oauth",
        type: "message",
        role: "assistant",
        model: "claude-sonnet-4-6",
        content: [{ type: "text", text: "UPSTREAM_OAUTH_RESULT" }],
        stop_reason: "end_turn",
        stop_sequence: null,
      ...
      const proc = await startTestServer(ANTHROPIC_PORT, {
        ...
        CLAUDE_CODE_OAUTH_TOKEN: "oauth-upstream-secret",
      });
      try {
        const res = await httpPost(
          `http://localhost:${ANTHROPIC_PORT}/v1/messages`,
        ...
        const body = JSON.parse(res.body);
        assert.equal(res.status, 200);
        assert.equal(body.content[0].text, "UPSTREAM_OAUTH_RESULT");
        assert.equal(upstream.requests.length, 1);
        assert.equal(upstream.requests[0].headers.authorization, "Bearer oauth-upstream-secret");
        assert.equal(upstream.requests[0].headers["x-api-key"], undefined);
        assert.equal(upstream.requests[0].headers["anthropic-beta"], "oauth-2025-04-20");
      } finally {
        proc.kill("SIGKILL");
        await sleep(300);
        if (fs.existsSync(fakeClaude)) fs.unlinkSync(fakeClaude);
